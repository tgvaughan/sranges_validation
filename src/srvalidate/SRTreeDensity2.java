...
public class SRTreeDensity2 extends TreeDistribution {
 ...
 //   protected RealParameter sRanges;
    ...
    public double p(double t, double c1, double c2) {
        return 1 + (- (lambda.getValue() - mu.getValue() - psi.getValue()) + c1 * ((Math.exp(- c1 * t) * (1 - c2) - (1 + c2)) / (Math.exp(- c1 * t) * (1 - c2) + (1 + c2))));
    }
    ...
    private void setPContribution(Node i, Node j, double logP, double c1, double c2) {
            ...
            if (i.getHeight() < (double) j.getMetaData("oldestFossil")) {
                ...
                for (Node k : tree.getNodesAsArray()) {
                    if (k.getHeight() < (double) j.getMetaData("oldestFossil")) {
                        if (k.getHeight() > (double) i.getMetaData("oldestFossil")) {
                            return;
                        }
                    }
                }
                ...
                while (node.getChild(0) != null) {
                    if (node == j) {
                        logP += Math.log(lambda.getValue()) + Math.log(P(i.getHeight(),c1,c2) - P((double) j.getMetaData("oldestFossil"),c1,c2));
                        break;
                    }
    ...
    public double calculateLogP() {
        ...
        for (Node node : tree.getNodesAsArray()) {
            ...
            if (node.getMetaData("reaction") == "psi") {
                ...
                if (node.getMetaData("oldestFossil").equals(node.getHeight())) {
                    k += 1;
                ...
                // if the stratigraphic range have different start and end times
                    ...
                    double startTime = (double) node.getMetaData("oldestFossil");
            ...
            //rho sampling
            else if (node.isLeaf() && node.getMetaData("rho") == "true") {
                l += 1;
                // if the stratigraphic range is represented by a single fossil
                if (node.getMetaData("oldestFossil").equals(node.getHeight())) {
                    k += 1;
    ...
    public static void main(String[] args) {
        ...
        density.psiInput.setValue(lambdaParameter, density);
